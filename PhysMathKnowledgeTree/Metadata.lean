namespace PhysMathKnowledgeTree

/-- Schema version of the canonical JSON graph shipped with this package. -/
def graphSchemaVersion : String := "0.5.0"

/-- Schema version of the deletion-safe source-curation ledger. -/
def curationSchemaVersion : String := "1.2.0"

/-- Application/repository release version. -/
def applicationVersion : String := "2.4.0"

/-- Stable repository identifier used by generated tooling. -/
def repositorySlug : String := "lluiseriksson/physmath-knowledge-tree"

end PhysMathKnowledgeTree
